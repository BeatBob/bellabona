...
> **Live preview:** https://bellabona-sable.vercel.app/
...
| LogoBar / Stats / Meals / Features / CtaBanner / Steps / Testimonial / Faqs   | ISR                     | Bundled in the same homepage GROQ query.                |
...
- **`generateMetadata`** in `app/(site)/page.tsx` reads `seo.metaTitle`, `seo.metaDescription`, `seo.ogImage`, `seo.canonicalUrl` from Sanity. No meta tags hardcoded in `layout.tsx`. `metadataBase` is set in the root layout so OG/canonical resolve correctly.
- **Organization JSON-LD** inlined as `<script type="application/ld+json">`:
...
- **FAQPage JSON-LD** generated from `homepage.faqs.items` and emitted alongside Organization — eligible for SERP FAQ rich results.
- **`robots.txt`** and **`sitemap.xml`** generated by `app/robots.ts` and `app/sitemap.ts` (Next.js metadata files). `/studio` and `/api/` are disallowed; sitemap drives off `LOCALES` so DE adds itself when activated.
...
```
app/
  layout.tsx                       # root, sets metadataBase, no hardcoded meta
  globals.css                      # Tailwind v4 @theme tokens
  robots.ts                        # /robots.txt
  sitemap.ts                       # /sitemap.xml
  (site)/
    layout.tsx                     # Nav + Footer + main padding for fixed header
    page.tsx                       # Server Component, ISR, fetches Sanity, JSON-LD
  api/revalidate/route.ts          # secret-gated revalidateTag endpoint
  studio/[[...tool]]/page.tsx      # embedded Sanity Studio
components/
  sections/                        # Hero, LogoBar, Stats, Meals, Features,
                                   # CtaBanner, Steps, Testimonial, Faqs
...
lib/
  ...
  jsonld.ts      # Organization + FAQPage schema generators
scripts/
  ...
  seed-faqs.mjs            # patch FAQ items modeled on the reference image
  seed-meals.mjs           # patch meals showcase (text only — upload images via Studio)
sanity.config.ts
```
